...
export default function CheckoutPage({ params }: { params: { brandSlug: string } }) {
  ...
  useEffect(() => {
    ...
    if (currentBrand) {
      ...
      const cartData = localStorage.getItem(`cart_${brandSlug}`)
      if (cartData) {
  ...
  const handleUpdateQuantity = (productId: string, variantId: string, newQuantity: number) => {
    ...
    setItems(prev => {
      ...
      // Update localStorage
      localStorage.setItem(`cart_${brandSlug}`, JSON.stringify(updated))
  ...
  if (items.length === 0) {
    return (
      <div className="container mx-auto p-8 text-center">
        ...
        <Button onClick={() => router.push(`/${brandSlug}`)}>
